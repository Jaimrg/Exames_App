...
<head>
  ...
  <title>Tela Candidatos | INATTER</title>
...
<body class="text-left" style="background-color: green;">
  <div class="app-admin-wrap layout-horizontal-bar">
    ...
    <div class="main-content-wrap d-flex flex-column">
      ...
      <div class="main-content">
        <h1 style="font-weight: bold; color: white!important;">Lista de Candidatos ao Exame</h1>
        <div class="row">
  ...
  <script>
    ...
    async function API(){
    let host = 'http://192.168.16.119:5000/examinandos';
    let data = {"examinando_id":"", "escola_id":"", "local_id":""}
      ...
      .then((data) => {
        ...
        data.opcoes.forEach(cand => {   
          //console.log(JSON.parse(exame.respostas).resultado);  
          console.log(cand);
          let id = cand.numero_documento;//JSON.parse(exame.respostas).resultado;  
          let categoria = cand.categoria;//JSON.parse(exame.respostas).pontuacao;
          let data_hora = cand.data_exame +' '+cand.hora_exame;
          let estado = cand.estado;
          tbodyHtml += 
              `<tr>
                <td class="b_left">${id}</td>
                <td>${data_hora}</td>
                <td>${categoria}</td>
                <td ${estado == '0' ? 'class="b_right "' : "" || estado == '0' ? 'class="b_right"' : ""}>${estado == '1' ? "<p class='badge badge-pill badge-success even-larger-badge'>Examinado</p>" : "<p class='badge badge-pill badge-danger'>Pendente</p>"}</td>
              </tr>`;
